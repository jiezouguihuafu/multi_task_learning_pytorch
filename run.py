...
class BaseArguments(object):
    def __init__(self):
        ...
        self.task_data = {
            'three': {'eval': 'three_eval.txt', 'train': 'three_train.txt', "label": "three_label.txt"},
            'nine': {'eval': 'nine_eval.txt', 'train': 'nine_train.txt', "label": "nine_label.txt"},
        }
...
def preprocess(args: Any, task: str):
    ...
    eval_df = get_df(args.data_dir, data_name['eval'])
    ...
    train_precessed, _ = train_val_split(total_precessed_data,args.train_val_split_ratio)
    eval_precessed = convert_df_to_inputs(task, tokenizer, eval_df, label2id)
    ...
    cache_data_path = {
        ...
        'eval': os.path.join(args.data_save_dir, task, 'eval.pt')
    }
    ...
    torch.save(eval_precessed, cache_data_path['eval'])
...
class Trainer:
    ...
    def train(self):
        ...
        eval_dataloader = self.get_dataloader(self.eval_dataset)
        ...
        logger.info("  Eval Num examples = %d", len(eval_dataloader))
        ...
        for epoch in range(self.args.num_train_epochs):
            ...
            for step, inputs in enumerate(train_dataloader):
                ...
                if global_step % self.args.logging_steps == 0:
                    ...
                    metrics = self.evaluate(dataloader=eval_dataloader)
    ...
    def evaluate(self, dataloader: DataLoader) -> Dict[str, float]:
        output = self._prediction_loop(dataloader)
        logger.info(output.metrics)
        return output.metrics

    def _prediction_loop(self, dataloader: DataLoader) -> PredictionOutput:
        model = self.model
...
def get_data():
    cache_data_path = {}
    if not os.path.exists(base_arguments.data_save_dir) or base_arguments.overwrite_cache:
        for id_, task in base_arguments.task_id_to_name.items():
            task_cache_data_path = preprocess(base_arguments, task)
            cache_data_path[id_] = task_cache_data_path
    else:
        for id_, task_name in base_arguments.task_id_to_name.items():
            cache_data_path[id_] = {
                'train': os.path.join(base_arguments.data_save_dir, task_name, 'train.pt'),
                'eval': os.path.join(base_arguments.data_save_dir, task_name, 'eval.pt')
            }

    multi_task_train_dataset = {}
    multi_task_eval_dataset = {}

    for id_, cache_path in cache_data_path.items():
        multi_task_train_dataset[id_] = SingleTaskDataset(cache_path['train'])
        multi_task_eval_dataset[id_] = SingleTaskDataset(cache_path['eval'])

    train_dataset = MultiTaskDataset(multi_task_train_dataset)
    eval_dataset = MultiTaskDataset(multi_task_eval_dataset)
...
def train():
    ...
    trainer = Trainer(model=model, args=base_arguments, train_dataset=train_dataset, eval_dataset=eval_dataset, compute_metrics=compute_metrics)
